"""Ecoacostics dataset provided A. Eldridge et al.
    ...
    Branch taken from and deveoped by Kieran Gibbs
    allows for sample idx to be loaded with each data point in the
    dataloader.
this implementation is from this branch https://github.com/wearepal/conduit/tree/feature/add-side-information-to-ecoacoustic-dataset
thank you to m4gpi user for this implementation, very valuable in plotting spectrograms with its respective information
"""
...
from conduit.data.structures import BinarySample, TernarySample
...
class Ecoacoustics(CdtAudioDataset[SampleType, Tensor, Tensor]):
    ...
    def __init__(
        ...
        *,
        ...
        #s = torch.as_tensor(x.copy())
        #y = torch.as_tensor( self._label_encode(self.metadata[self.target_attrs], inplace=True).to_numpy() )
